...
const ChannelPage = () => {
    ...
    const getLatestMessages = async (channelName) => {
        try {
            const response = await axios.get(`http://localhost:8080/api/channel/${channelName}/latest`, {withCredentials: true});
...
function Channels({channelName, channels, getChannelData, isAdmin, loggedUser, notifChannel, channelType, handleChannelTypeChange}) {
...
function ChannelList({channelName, channels, getChannelData, channelType, isAdmin, loggedUser, notifChannel}) {
...
function ChannelButton({channelName, getChannelData, channelKey, channel, isAdmin, loggedUser, notifChannel,}) {
...
function Channel({messages, newMessage, setNewMessage, handleSendMessage, handleDeleteMessage, isAdmin, channelName, loggedUser}) {
...
function Members({channelName, users, isAdmin, loggedUser, setIsAdmin}) {
    ...
    const changeUserRole = async (targetUsername, newRole) => {
        try {
            const response = await axios.put(`http://localhost:8080/api/admin/updateRole`, null, {
                params: {
